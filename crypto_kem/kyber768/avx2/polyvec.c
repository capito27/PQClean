...
/*************************************************
* Name:        PQCLEAN_KYBER768_AVX2_polyvec_du_compress
*
...
void PQCLEAN_KYBER768_AVX2_polyvec_du_compress(uint8_t r[KYBER_POLY_DU_VECBYTES], const polyvec *a) {
    unsigned int i;
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_AVX2_poly_du_compress(r + i * KYBER_POLY_DU_BYTES, &a->vec[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_KYBER768_AVX2_polyvec_du_decompress
*
* Description: De-serialize and decompress with factor du=9 vector of polynomials;
*              approximate inverse of PQCLEAN_KYBER768_AVX2_polyvec_du_compress
*
...
void PQCLEAN_KYBER768_AVX2_polyvec_du_decompress(polyvec *r, const uint8_t a[KYBER_POLY_DU_VECBYTES]) {
    unsigned int i;
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_AVX2_poly_du_decompress(&r->vec[i], a + i * KYBER_POLY_DU_BYTES);
    }
}

/*************************************************
* Name:        PQCLEAN_KYBER768_AVX2_polyvec_dv_compress
*
...
void PQCLEAN_KYBER768_AVX2_polyvec_dv_compress(uint8_t r[KYBER_POLY_DV_VECBYTES], const polyvec *a) {
    unsigned int i;
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_AVX2_poly_dv_compress(r + i * KYBER_POLY_DV_BYTES, &a->vec[i]);
    }
}

/*************************************************
* Name:        PQCLEAN_KYBER768_AVX2_polyvec_dv_decompress
*
* Description: De-serialize and decompress with factor dv=3 vector of polynomials;
*              approximate inverse of PQCLEAN_KYBER768_AVX2_polyvec_dv_compress
*
* Arguments:   - polyvec *r:       pointer to output vector of polynomials
*              - const uint8_t *a: pointer to input byte array
...
void PQCLEAN_KYBER768_AVX2_polyvec_dv_decompress(polyvec *r, const uint8_t a[KYBER_POLY_DV_VECBYTES]) {
    unsigned int i;
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_AVX2_poly_dv_decompress(&r->vec[i], a + i * KYBER_POLY_DV_BYTES);
    }
}
